...
$captcha=$d['contactForm']['captchaType']??'none';if($captcha==='integrated'&&empty($_POST['human']))exit('Verificación requerida.');if($captcha==='google_v2'){$secret=$config['recaptcha']['secret_key']??'';$token=$_POST['g-recaptcha-response']??'';if(!$secret||!$token)exit('reCAPTCHA no está configurado.');$context=stream_context_create(['http'=>['method'=>'POST','header'=>'Content-Type: application/x-www-form-urlencoded','content'=>http_build_query(['secret'=>$secret,'response'=>$token,'remoteip'=>$_SERVER['REMOTE_ADDR']??'']),'timeout'=>15]]);$result=json_decode((string)file_get_contents('https://www.google.com/recaptcha/api/siteverify',false,$context),true);if(empty($result['success']))exit('No fue posible verificar reCAPTCHA.');}
